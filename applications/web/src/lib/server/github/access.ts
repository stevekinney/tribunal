...
// Configuration
const GITHUB_ACCESS_CACHE_TTL = parseInt(env.GITHUB_ACCESS_CACHE_TTL ?? '300', 10); // 5 min default
...
function shouldCacheDenial(
  ...
  switch (reason) {
    ...
    case 'no_access':
      ...
      return { cache: true, ttl: GITHUB_ACCESS_CACHE_TTL };
...
async function getCachedAccessEntry(
  ...
  return getCached<GitHubAccessCacheEntry>(key);
}
...
async function getCachedAccess(
  ...
  const entry = await getCachedAccessEntry(userId, repositoryId);
...
async function cacheResult(
  ...
  if (result.allowed) {
    ...
    await setCache(key, entry, GITHUB_ACCESS_CACHE_TTL);
  } else {
    // Check if we should cache this denial
    const { cache, ttl } = shouldCacheDenial(result.reason, scopes, lastSuccessAt);
    if (cache && ttl > 0) {
      const entry: GitHubAccessCacheEntry = {
        result,
        cachedAt: Date.now(),
...
async function resolveAndVerifyAccess(
  ...
  const existingEntry = await getCachedAccessEntry(userId, repositoryId);
  ...
  if (publicResult?.allowed && publicResult.visibility === 'public') {
    await cacheResult(userId, repositoryId, publicResult, scopes, lastSuccessAt);
    return publicResult;
  }
  ...
  if (!authResult.allowed && authResult.reason === 'invalid_token') {
    ...
    await invalidateGitHubAccessCache(userId); // Clear all cached access for this user
    return authResult;
  }
  ...
  if (!authResult.allowed && authResult.reason === 'no_access') {
    if (scopes.hasRepo) {
      ...
      await cacheResult(userId, repositoryId, authResult, scopes, lastSuccessAt);
...
async function verifyWithCircuitBreaker(
  ...
  const promise = resolveAndVerifyAccess(userId, repoId).finally(() => {
 ...
 * @param options - Optional settings (e.g., skipCache for admin/sensitive actions)
 */
export async function verifyGitHubRepositoryAccess(
  ...
  if (!options?.skipCache) {
    const cachedResult = await getCachedAccess(userId, repositoryId);
    if (cachedResult) {
      // Log cached access
      logAccessAttempt({
        timestamp: new Date(),
  ...
  const result = await verifyWithCircuitBreaker(userId, repositoryId);
 ...
 */
export async function invalidateAllAccessCacheForRepo(repositoryId: number): Promise<void> {
 ...
 */
export async function markGitHubTokenInvalid(userId: number): Promise<void> {
  try {
    await db
