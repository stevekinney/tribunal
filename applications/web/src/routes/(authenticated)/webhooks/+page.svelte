<script lang="ts">
  ...
  // Partition (not duplicate) subscribed events by whether Tribunal has ever
  // received one -- "subscribed but zero received" and "received at least
  // once" are different operational situations that otherwise render
  // identically. `receivedEventTypeSet` is an unbounded, all-time check (see
  // `getWebhookEventFilterOptions`'s doc comment), so membership here proves
  // only that a delivery arrived at some point, not that it is still
  // arriving now -- deliberately not labeled "active" or "currently
  // receiving" in the UI below.
  const everReceivedSubscribedEventTypes = $derived(
    ...
    {:else}
      ...
      {#if data.subscribedEventTypes.length > 0}
        <Card
          ...
          headingLevel={2}
        >
          {#if everReceivedSubscribedEventTypes.length > 0}
            <p class="subscribed-events-heading">Received at least once:</p>
            <div class="subscribed-events">
              {#each everReceivedSubscribedEventTypes as eventType (eventType)}
