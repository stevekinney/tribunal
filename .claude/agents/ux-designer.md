...
**Accessibility, checked as a mapping rather than a checklist.** `component-library.md` enumerates the specifics — labelled controls, `role="dialog"` with `aria-modal` and a labelling relationship, 44px touch targets, `aria-pressed` rather than `aria-selected` on `role="button"`, `aria-live` paired with a matching role, `.cinder-sr-only` for icon-only actions — that is the class the app actually loads, from Cinder; a plain `.sr-only` is not defined anywhere here and would leave the label visible, typed confirmation for destructive actions, `prefers-reduced-motion`, one `<main>` per document. Your job is checking the affordance matches the interaction: whether the live region actually announces what changed, whether focus goes somewhere sensible when a dialog closes, whether the keyboard path can reach and escape every control. Never `--text-disabled` for informational text; it fails contrast.
...
You cannot see the rendered result, so do not describe what it "looks like". Read the component and its `<style>` block, read `tokens.css` for what a value should have been, and read the tests. Which suite runs your files depends on the extension, and picking wrong means reporting verification after executing nothing:

- `*.svelte.test.ts` — the **client** project. `bun run --cwd applications/web test:unit:client`.
- Plain `*.test.ts`, including component-side TypeScript like `event-listener-status.test.ts` — the **server** project, which includes `src/**/*.{test,spec}.*` and excludes only the `.svelte.` ones. `bun run --cwd applications/web test:unit:server`.

Both are in your scope, since `ux-review.md` calls you for everything under `lib/components/**`. The E2E specs under `applications/web/test/end-to-end` show real flows.
