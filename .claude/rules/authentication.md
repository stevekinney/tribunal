...
One deliberate exception: the MCP + OAuth mount's combined handle (`createMcpHandle`) consumes the locals `authHandle` populates but **excludes** the dev-bypass synthetic user (`TRI-45`). The bypass is a convenience for browsing the authenticated UI; it must never own an OAuth grant on the mounted surface, because an armed bypass on an externally reachable dev server would otherwise mint real tokens. So the mounted OAuth surface is unreachable via the bypass even though the mount sits after `devAuthBypassHandle`.
