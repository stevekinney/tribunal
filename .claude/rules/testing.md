...
- In `applications/web`, express the inapplicable runtime with `test.runIf(...)`, not an early `return`. That workspace sets `expect: { requireAssertions: true }`, under which a test returning before it asserts fails. A skip reports the truth; a vacuous pass does not.
