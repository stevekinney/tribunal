...
**Only hand a parser languages it can read.** This rule previously said the opposite — name what is *not* source, so an unrecognised file is inspected rather than skipped — on the grounds that an allowlist cannot predict every runnable spelling. The premise was that parsing a non-module is harmless, and it is not: TypeScript's parser recovers rather than failing, so `# import 'bun:test'` in any hash-commented language becomes a real `ImportDeclaration` and an always-on hook rejects every commit. Three review rounds added the languages observed so far before the shape was changed; the tail does not end, since Lua's `require('bun:test')` is valid Lua that parses as a JavaScript call. `isScannableFile` now accepts known JavaScript and Svelte extensions plus extensionless paths, whose shebang decides once the contents are read. Check the repository's actual extension inventory before reopening this — it was a stand-off between two plausible principles until `git ls-files` showed there is no exotic-but-JavaScript file to protect.
...
**One resolver per question.** When two code paths answer the same question, a correction lands in one of them and the other becomes the next finding. Three separate reports on #327 were this: a source check that rejected an identifier the sibling path already resolved, a name comparison that pre-empted the resolver written for exactly that case, and two private copies of "a value can come from either branch" — the ternary was added to both by hand, so `||`, `&&`, and `??` went into neither. Each fix was deleting the second path, not extending it. When a report says "this route handles X but that route does not", the defect is that there are two routes.

**A guard's input filter is part of the guard.** Everything downstream is invisible to inputs the filter drops, so a cheap pre-filter can be the whole hole while the logic reads as careful. Selecting files by `text.includes('spawnSync')` excluded every caller reaching the spawner through a re-export, and reading argument index 2 unconditionally meant a `Bun.spawnSync(command, options)` call — this repository's own — was inspected at a position holding nothing. Ask "which inputs never reach this code, and where does this look?" as its own review pass; a check can be green because it is looking in the wrong place.
...
**Gate the commit on the checks rather than running them beside it.** Make the commit conditional on the exit codes, and include every gate CI runs — `bun run check` is the one most easily forgotten, and its CI job is named "Type Check". Note also that a background task's completion notification reports the wrapper's exit status, not the command's. The same rule governs polling CI, with a caveat worth more than the documentation: `gh pr checks` documents exit 8 for pending, 0 for all-pass, and 1 for a terminal failure, but observed against this repository the plain form returned **1 while seven checks were pending**, and the `--json` form returns **0 while pending** — so in neither spelling is the exit code the pending signal. Poll `--json name,bucket` and count rows whose `bucket` is `pending`, treating an empty response as pending too, since the suite for a just-pushed commit does not exist yet. A wait script that scraped the table instead read zero pending while a check was plainly running and took a merge-gate reading five minutes early; the first two replacements for it were written from the documented exit codes and were wrong in the same direction.
