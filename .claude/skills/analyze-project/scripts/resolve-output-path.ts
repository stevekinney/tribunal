...
function pickPreferredPath(paths: string[]): string | undefined {
  ...
  const scoredPaths = paths.map((path) => {
    ...
    let score = 10;
    if (path === '.claude/project-context.md' || path === '.claude/multi-agent-plan-authoring-prompt.md') {
      score = 100;
...
async function findExistingArtifactPath(kind: ArtifactKind, cwd: string): Promise<string | undefined> {
  ...
  const includedPaths = paths.filter(isIncludedPath).sort((leftPath, rightPath) =>
