...
- Added reviewer image self-check coverage for runtime imports used by `runner/run-agent.mjs`,
  including `@anthropic-ai/claude-agent-sdk` and `@tribunal/agents`.
- Built and ran the reviewer image locally with `docker build -f deployment/containers/reviewer.Dockerfile -t tribunal-reviewer:issue-15 .` and `docker run --rm tribunal-reviewer:issue-15`.
...
- Build every deployment image against a running Docker daemon in CI.
