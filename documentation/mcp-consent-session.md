...
**One ordering requirement for TRI-31, and it is a requirement rather than an open question.** Tribunal's MCP and OAuth routes are mounted by a handle inserted into `hooks.server.ts`'s `sequence()`. (An earlier draft named Protokit's `createApplicationMount()` here, written when the engine was to be forked and its application seam reused. `documentation/mcp-integration-orchestration.md` rejects that seam: the mount is Tribunal's own, built on the published engine. The requirement below is unchanged by that — it constrains where the mount sits, not what supplies it.) If it is placed _before_ `authHandle`, an intercepted `/oauth/authorize` request can be answered by the mount without ever invoking the downstream `resolve`, so `authHandle` never runs and `event.locals.user` is never populated. The consent page would then be handling an unauthenticated request, or would need its own second validation path — the duplicate JWT validator this project's locked decisions rule out.
...
The dev auth bypass is a deliberate exception (`TRI-45`). `devAuthBypassHandle` populates a synthetic user under `DEV_AUTH_BYPASS=1` for browsing the authenticated UI, but `createMcpHandle` does **not** prime that synthetic identity into the mount — a bypass user must never own an OAuth grant, because an armed bypass on an externally reachable dev server (a tunnel) would otherwise complete consent and mint a real token. So under `DEV_AUTH_BYPASS=1` the mounted `/oauth/authorize` is intentionally unauthenticated; only a real session reaches consent.

The unauthenticated response there is **not** the ordinary `/login` redirect. The bypass user is authenticated on `/login` too, whose load would redirect it straight back to `returnTo`, bouncing between `/oauth/authorize` and `/login` forever. So `handleUnauthenticatedAuthorization` returns a **terminal 403** when the bypass is armed, and the ordinary `302 → /login?returnTo=` only for a genuinely unauthenticated real request.

Two things need a test: the mount follows every identity-populating handle so a mounted consent request from a real session sees `event.locals.user`, and under `DEV_AUTH_BYPASS=1` the mounted `/oauth/authorize` returns a terminal 403 (not a `/login` redirect) rather than authenticating the synthetic user. The ordering is a single line in `hooks.server.ts` that a later edit can silently invert with no other symptom.
...
### Keeping the JWT alive across the consent window (resolved by TRI-122)

The 5-minute refresh above is not ambient. `useNeonSessionRefresh` is started by exactly two callers—`routes/(authenticated)/+layout.svelte` and `routes/onboarding/+page.svelte`—and its own contract says every authenticated page must opt in. A consent page served through the hook-mounted application does not inherit either, and it ships zero client JavaScript by TRI-40 AC4, so it cannot start the refresh itself.
...
**TRI-122 resolves this server-side, without any client JavaScript.** The candidates were: re-mint the cookie at `GET /oauth/authorize`, tolerate a just-expired JWT on the approve/deny POST, or shorten the transaction TTL to the JWT lifetime. Re-minting is infeasible—a fresh JWT is minted only by Neon Auth's Better Auth `/get-session`, whose long-lived session lives client-side, so Tribunal's server holds no credential to mint one; the bridge's `refreshOnly` path only re-sets the cookie expiry to the token's own `exp` and cannot extend a JWT's validity. Shortening the TTL cannot help alone, because the JWT's remaining lifetime at render is unknown.

The chosen mechanism is the second: `resolveIdentityBinding` (`lib/server/oauth/identity.ts`) verifies the JWT with up to the transaction's own 10-minute TTL of `exp` leeway—but **only on the approve/deny POST**, never on the initiating GET. This closes the window without opening a new one: the library's `consume` independently rejects any transaction whose `expiresAt <= now`, so a JWT stale by more than the transaction TTL can only ever meet an already-expired transaction, and the transaction-bound one-time CSRF token proves the POST came from the same browser that authenticated at render. Signature, issuer, audience, and `nbf` are still verified in full—only `exp` gains leeway; a session that lapsed before consent even began (the GET) still signs in again. A dev-auth-bypass session sets no Neon Auth cookie, so the leeway grants it nothing (TRI-45). The threading is a bounded `clockToleranceSeconds` option on `verifyNeonAuthToken`/`validateNeonSessionFromToken`; `authHandle` and the session bridge pass none and stay strict.

Two halves are required, because server-side leeway is useless if the browser has already dropped the cookie. `setNeonAuthTokenCookie` otherwise sets the cookie's `expires` to the JWT's exact `exp`, so a real browser would stop sending the token the instant it lapses—before the POST-only tolerance is ever reached. The cookie is therefore retained for the same grace window past the JWT's `exp` (`neonAuthConsentGraceSeconds`, shared with the verification leeway so the two cannot drift), so the browser still holds and sends the lapsed token on the approve/deny POST. The retained cookie is only ever accepted on that consent POST within the grace; `authHandle` still rejects and clears an expired JWT on any other request.
...
- `ENCRYPTION_KEY`—AES key encrypting OAuth connection tokens at rest (GitHub access and refresh tokens in `oauthConnection`), used by `applications/web/src/lib/server/encryption.ts` and `packages/github/src/reviews/read-tokens.ts`. Encryption, not signing; no rotation-overlap variable. **Like `PROXY_SIGNING_KEY`, it is shared across applications rather than owned by one:** `createEngineGithubContext` and `createProxyGitHubCredentialResolver` both pass it into the shared GitHub token reader, and `DEPLOYMENT.md` requires the same value across web, engine, and proxy. Rotating one service independently makes the shared encrypted-token cache reject and replace entries repeatedly, so provisioning and rotation are a three-application operation.
...
What would resolve it: inspect an actual minted token, or check Better Auth's JWT plugin configuration for a session-instance claim. That was not done here and cannot be answered from this repository alone.

**How this resolves against the approval, since the two readings contradict each other.** An earlier revision said "if the approver wants the cross-session protection preserved, this should be resolved before TRI-31", while the status header says the question is carried forward to TRI-31. Approving TRI-25 recorded no separate answer, so neither "TRI-31 is blocked" nor "the defence was declined" is established — and the second is the dangerous reading, because an implementer could bind to `userId` alone believing the reduction was accepted on purpose.

The reconciliation, which needs no approved answer: **the investigation is TRI-31's first acceptance criterion, not a prerequisite to it.** TRI-31 inspects a minted token or Better Auth's JWT plugin configuration and **records what it finds** before implementing the binding. TRI-31 stays `Ready`; the question travels with it.

**What TRI-31 does with the finding, and this is deliberately narrow.** If no refresh-stable per-login claim exists, `userId` alone is the only option — implement the design above and record that the reduction is forced rather than chosen, so nobody re-litigates it later.

If such a claim _does_ exist, **TRI-31 records that and stops there; it does not change the binding.** Binding to a session-instance claim means persisting and comparing it, which requires a column and a consume predicate this document's approved design explicitly excludes — no new column, `sessionTokenHash` removed, the two identity predicates collapsed to `userId` alone. TRI-31 cannot both follow that design and bind to a claim, and an earlier revision of this paragraph told it to do exactly that. Recovering the cross-session protection is therefore an **amendment to this approved decision**, with its own column, predicate, and cross-session test specified — a decision issue, not something settled inside TRI-31's pull request.

**What is not acceptable is implementing the `userId` binding without looking**, which silently converts an open question into a permanent reduction on a security-relevant bound. Looking is cheap; the finding is what tells the approver whether an amendment is worth filing.
