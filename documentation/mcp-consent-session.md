...
**How this resolves against the approval, since the two readings contradict each other.** An earlier revision said "if the approver wants the cross-session protection preserved, this should be resolved before TRI-31", while the status header says the question is carried forward to TRI-31. Approving TRI-26 recorded no separate answer, so neither "TRI-31 is blocked" nor "the defence was declined" is established — and the second is the dangerous reading, because an implementer could bind to `userId` alone believing the reduction was accepted on purpose.

The reconciliation, which needs no approved answer: **the investigation is TRI-31's first acceptance criterion, not a prerequisite to it.** TRI-31 inspects a minted token or Better Auth's JWT plugin configuration and records what it finds before choosing a binding target. If a refresh-stable per-login claim exists, TRI-31 binds to it and the cross-session protection is recovered. If none exists, `userId` alone is the only option and the reduction is forced rather than chosen — record that explicitly. **What is not acceptable is binding to `userId` without looking**, which would silently convert an open question into a permanent reduction. TRI-31 stays `Ready`; the question travels with it.
