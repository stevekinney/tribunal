...
**The bridge has been retired.** TRI-27 shipped `@tribunal/mcp` as a temporary copy of the engine. TRI-80 deleted that workspace, pinned `@lostgradient/mcp@0.2.0`, and repointed every consumer. The copy was not wasted—it proved the engine has zero database or web coupling, and its 31 substantively changed files became the requirements list for the published package (runtime-agnostic environment handling, no `bun:test`, a version that survives bundling, and an injectable logger).
...
**The released capabilities are now in use.** The complete chain has landed: TRI-72 and TRI-73 supplied the library seams, TRI-74 published them, and TRI-80 replaced the local bridge with `@lostgradient/mcp@0.2.0`. Downstream work must build against that installed package rather than recreate or patch a local engine copy.
...
- **`sslmode=verify-full`, never `require`** (TRI-44). `require` encrypts without verifying the certificate; `verify-ca` skips hostname verification. Amended by **TRI-124**: this only constrains the `node-postgres` raw-TLS path. A Neon host (`*.neon.tech`, `*.neon.build`) is exempt — it connects over `drizzle-orm/neon-http`'s HTTPS transport, where `sslmode` is inert and certificate verification happens at the HTTPS/fetch layer. Enforcing this invariant unconditionally crash-looped production for ~4 hours on 2026-09-09/10 when a real Neon production `DATABASE_URL` lacked `verify-full`.
