...
- **Move the issue to Done yourself**, with completion evidence attached. Stopping at In Review is not finishing. For a `type:release` issue, a merged pull request is never sufficient evidence — the deployed artifact or recorded transcript is.
  ...
  "Yourself" means the orchestrator, and only the orchestrator. This does not conflict with the `execute-plan` skill, which forbids updating a Linear ticket's status automatically: that boundary governs _delegated executors_, and it still holds here. A subagent, teammate, or workflow agent must never move a ticket, and none of them are given Linear write access in this project — they report their result upward instead. The orchestrator is the primary coordinator and the sole Linear writer, reads the evidence, and makes the transition itself. If you are executing an issue rather than orchestrating the graph, `execute-plan`'s rule applies to you unchanged: report and hand back.
