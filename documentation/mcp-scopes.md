...
**The scope vocabulary belongs to Tribunal, not the engine.** TRI-80 removed the temporary `packages/mcp` bridge and pinned `@lostgradient/mcp@0.2.0`. Define Tribunal's vocabulary through the installed package's consumer-supplied scope API; do not recreate a local engine package or add Tribunal scopes to Protokit's template vocabulary.
...
_Update — TRI-126 reversed the resource half of this (prompts are still absent)._ Tribunal now exposes exactly one fixed-URI resource, `tribunal://review-runs` (scope `reviews:read`), returning the caller's own review runs as a JSON document — the same lifecycle-and-cost projection `list_review_runs` serves, not a new data surface. The objection above — that the engine "cannot enforce" resource authorization for a consumer — turned out to be wrong for this engine: the published serving handler gates `resources/read` on the resource's `requiredScope` exactly as it gates `tools/call`, and a token lacking `reviews:read` is refused (`review-run-resource.mount.test.ts` proves both the grant and the refusal against the real mount). Adding the resource also flips the modern era's advertised `resources.subscribe` capability on, which is why it arrives paired with a producer rather than as a static document: the web process calls `notifyReviewRunsChanged(userId)` when a run's lifecycle status changes through cancellation (`operator.stopRun`), and that reaches the mount's `publishUserResourceUpdate`, which emits `notifications/resources/updated` to a subscribed client. The producer lives in the web process deliberately — web is the only process that mutates `tribunalRun` status on a path a subscribed user is watching, and it is pinned to a single instance, so the in-process handler cache reaches the subscriber without cross-instance messaging. Richer, engine-written updates (a run's findings or progress) are **not** covered here: findings are written only in the engine process, so delivering them to a web-held subscription needs the cross-instance messaging seam (TRI-49/56), and that is deferred and tracked separately. See `applications/web/src/lib/server/mcp/resources/review-run-resource.ts` and `resource-updates.ts`.
...
**A review run returns status, timing, and cost estimate — and not its trigger, its reviewed commit, or its posted-comment count.** Review feedback on the implementing pull request found the first projection wider than this scope's consent sentence, and it was right: a token holding `reviews:read` alone would otherwise learn which commit a pull request sat at and how much review traffic it drew, without the pull request capability the user is asked to approve separately. Those three fields are gone rather than the consent copy being widened, because that copy is TRI-24's approved text and this document gives no latitude to restate it — unlike `pull_requests:read`, where it named narrowing as an acceptable resolution.
...
**`readOnlyHint: true` means the tool changes none of the data these scopes govern, and one credential write is a named exception.** `getRepositoriesForUser` is called with `recordTokenInvalidation: false`, so an MCP read never writes `oauth_connection.status = 'invalid'` the way an interactive route does. What it does still reach is `getUserOctokit`, which rotates a token inside its expiry window and persists the new one. That write stays: it is credential maintenance the transport performs to make the read possible, it changes no repository, pull request, review, finding, or cost record, and it is invisible to the user. Suppressing it would make a call arriving inside the refresh window fail rather than succeed — a real behavioural cost for a definitional gain. Recorded here rather than left implicit, so a later reader does not build on "these tools never write" as an unqualified fact.
