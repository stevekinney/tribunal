...
**The read path is not self-gating, and any tool built on it must add the check itself.** `getRepositoryById` (`packages/github/src/repositories/service.ts:251-256`) takes only a repository id and performs an unscoped `select().from(repository).where(eq(repository.id, ...))`. There is no user parameter and no installation filter. `getOrCreateRepository` is an unscoped upsert, not a read at all. Tribunal's own web routes are safe because they call `userCanAccessRepository(user.id, repositoryId)` _separately_ before exposing the row.
...
Pull requests are never stored (`documentation/ARCHITECTURE.md`, Data Model section)—they are fetched live from the GitHub API through `packages/github/src/pull-requests/service.ts` (`listPullRequests`, `getPullRequest`, `getPullRequestOperationalStatus`). Content here is entirely author-controlled: title, description, diff, and comments come from whoever opened the pull request, not from Tribunal or the requesting user.
...
**But `getRunInspector` is not a lifecycle-only read, and reusing it wholesale would collapse this scope into the next one.** It queries agent descriptions and agent events alongside run rows (`applications/web/src/lib/server/review/operator.ts:778-862`), returning that content inside each agent run. A token holding only `reviews:read` would then receive material this document deliberately reserves for `review_findings:read`, defeating the prompt-injection boundary described below—the entire reason the two are separate scopes.

**Requirement for F2/O1: a `reviews:read` tool must use a lifecycle-only projection, or strip the separately scoped fields before returning.** `getRunsOverview` is the closer fit; `getRunInspector` must not be exposed unmodified.
...
A narrower default (for example, `repositories:read` and `reviews:read` only, withholding `pull_requests:read`, `review_findings:read`, and `cost_events:read` unless explicitly requested) reduces risk only for a client that sends an explicit `scope`. **Under the working assumption above—not a confirmed fact, and unconfirmed for every one of the four clients—**a narrower default would instead mean three of five scopes never work until a further Tribunal-side or client-side change adds explicit scope requests. If TRI-62 through TRI-65 find that a client does send an explicit `scope`, that client is unaffected by this default either way, and the argument for it weakens accordingly. It would also produce a worse failure mode than either extreme: some tools silently work and others silently 403, which is harder for a user to reason about than "I approved everything" or "nothing works."
