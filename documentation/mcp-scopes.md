...
Every one of the four target clients (Claude Code, Codex CLI, the Claude hosted connector, ChatGPT developer mode) is expected to omit `scope` on its authorize request—`documentation/mcp-integration-orchestration.md` states this for Tribunal's own client set, not just Protokit's. A default of "grant nothing" would mean every one of the four clients Tribunal must support gets a token that fails every tool call with `insufficient_scope`, on day one, with no code path that lets it ask for more—none of the four is expected to send an explicit `scope` parameter a Tribunal-side change could react to.
