...
| `issue_comment`               | `applications/web/src/routes/api/webhooks/github/handlers/issue-comment.server.ts`                       | Manual fallback validates known `created`/`edited`/`deleted` shapes with library guards; repository deliveries write `webhook_event`, extracting `issueNumber` and, for pull request comments, `prNumber`; every delivery is claimed.              | Human-authored pull request comment `created`/`edited` events and all pull request comment `deleted` events call `signalPullRequestEvent`, but issue-comment event types currently map to no durable `review_intent`; issue and pull request caches are invalidated.                                                                                                                                                                                                                                             | `applications/web/src/routes/api/webhooks/github/handlers/issue-comment.server.test.ts` covers pull request filtering, human create/edit, deletion, bot-created skip, and non-durable kick skip; `packages/github/src/webhooks/extract.test.ts` covers issue and pull request comment field extraction; `packages/github/src/webhooks/resource-invalidation.test.ts` covers issue-comment cache invalidation.                                                                                     | No — source and tests exist, but no production delivery evidence is checked in. |
