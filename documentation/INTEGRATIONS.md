...
Some call functions with names like `signalPullRequestEvent`, `signalPullRequestClosed`, and `signalManualReview` (from `@tribunal/github/pull-requests/state/workflow-signals`). Those functions now write idempotent `review_intent` rows only for event kinds the current review engine consumes: `pull_request.opened`, `pull_request.reopened`, `pull_request.ready_for_review`, `pull_request.synchronize`, `pull_request.closed`, `check_run.completed`, `check_run.rerequested`, `check_run.requested_action` with Tribunal's re-review identifier, and `check_suite.completed`/`rerequested`. Review, review-comment, review-thread, issue-comment, and base-branch push events are parsed and logged or used for cache/state maintenance, but they do not create durable review work today.
...
| ----------------------------- | -------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------- |
| `pull_request`                | `applications/web/src/routes/api/webhooks/github/handlers/pull-request.server.ts`                        | Typed router validates the payload; repository deliveries write `webhook_event` through `storeWebhookEvent`, extracting `prNumber` and sometimes `githubCreatedAt`; every delivery is claimed in `github_webhook_delivery`.           | `opened`, `reopened`, `ready_for_review`, `synchronize`, and `closed` write idempotent `review_intent` rows and kick the review engine when configured; all actions also run repository metadata handling, cache invalidation, and fire-and-forget PR-state tracking. | Handler tests cover lifecycle dispatch with mocks; `packages/github/src/pull-requests/state/workflow-signals.test.ts` proves durable intent writes; `packages/github/src/webhooks/extract.test.ts` uses schema fixtures for extraction.                                             | No — source and tests exist, but no production delivery evidence is checked in. |
...
| `check_run`                   | `applications/web/src/routes/api/webhooks/github/handlers/check-run.server.ts`                           | Typed router validates the payload; repository deliveries write `webhook_event`, extracting `commitSha`; non-completed, non-Tribunal rerun noise is pre-database-ignored after listener matching; every delivery is claimed.          | `completed` writes `commit_pushed` review intents for associated pull requests; `rerequested` and Tribunal `requested_action` write `manual` review intents; completed and rerun triggers kick the engine when durable work exists.                                   | Handler and shared dispatch tests cover completed and rerun fanout with mocks; workflow-signal tests prove `check_completed` and `manual` intent writes; resource invalidation tests cover check cache invalidation.                                                                | No — source and tests exist, but no production delivery evidence is checked in. |
| `installation`                | `applications/web/src/routes/api/webhooks/github/handlers/installation-lifecycle.server.ts`              | Typed router validates the payload; no top-level `repository` means the normal `webhook_event` storage path is skipped; every delivery is claimed.                                                                                    | `created` upserts an installation stub and starts fire-and-forget installation sync; `deleted`, `suspend`, and `unsuspend` update/delete installation state; `new_permissions_accepted` marks active and starts sync.                                                 | Handler tests cover lifecycle updates and sync dispatch with mocked records; lifecycle package tests cover deletion, status, repository removal, and cancellation behavior.                                                                                                         | No — source and tests exist, but no production delivery evidence is checked in. |
| `installation_repositories`   | `applications/web/src/routes/api/webhooks/github/handlers/installation-repositories-lifecycle.server.ts` | Typed router validates the payload; no top-level `repository` means the normal `webhook_event` storage path is skipped; every delivery is claimed.                                                                                    | `added` starts fire-and-forget installation sync; `removed` marks installation repositories inactive, cancels affected workflows, then starts sync; access and installation resource caches are invalidated.                                                          | Handler tests cover added/removed dispatch; access-invalidation tests use `github-webhook-schemas` fixtures for added/removed repository cache invalidation.                                                                                                                        | No — source and tests exist, but no production delivery evidence is checked in. |
...
| `push`                        | `applications/web/src/routes/api/webhooks/github/handlers/push-lifecycle.server.ts`                      | Typed router validates the payload; repository deliveries write `webhook_event`, extracting `ref` and `commitSha`; every delivery is claimed.                                                                                         | Fire-and-forget base-branch dispatch updates stored default-branch commit SHA and logs affected pull requests; push resource cache invalidation clears branch-head cache.                                                                                             | Handler tests cover dispatch invocation with mocks; `packages/github/src/webhooks/pr-state-dispatch.test.ts` covers base-branch behavior with fixture payloads.                                                                                                                     | No — source and tests exist, but no production delivery evidence is checked in. |
...
- The evidence gap for production payloads can only be closed by recording sanitized GitHub deliveries from production or by replaying sandboxed deliveries captured from GitHub, then asserting the concrete database/cache/review-intent effect for each event type.
