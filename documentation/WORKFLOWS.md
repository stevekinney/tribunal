...
`applications/web/src/routes/api/webhooks/github/+server.ts`. The webhook route is
responsible for the synchronous ingress work: verify, claim, store, route, invalidate
caches, update local state, and enqueue durable review intent rows for the current
review engine when an event is eligible.
...
2. **Claim the delivery** (`claimWebhookDelivery`) so duplicate deliveries from GitHub
   retries are processed at most once. Review-engine dispatch failures release that claim
   before returning 500 so GitHub redelivery can retry durable review-intent enqueue.
