...
**This procedure below only restores the container image.** `flyctl deploy --config deployment/fly/web.toml` reads that file from whatever checkout you run it from—it does not pin to a commit—so run it from a clean, up-to-date `main` checkout (`git status` clean, `git fetch origin main && git checkout origin/main`) rather than a stale local branch, so the config that ships is the one actually committed to `main`, not something older or locally modified. It does not touch Fly secrets (the app's _current_ secrets apply, not whatever was set historically). If the failure was caused by an image regression—bad application code, like the TRI-124 incident—this is the right tool, and current `main`'s config is the correct config to pair it with. If it was caused by a bad `web.toml` change (an invalid port, a broken health-check path) itself still present on `main`, or a bad secret, redeploying an old image on top of it reproduces or fails to fix that broken runtime state; instead revert or forward-fix the config/secret directly (`git revert` the offending commit and let the next `Deploy Production` run apply it, or `flyctl secrets set` the corrected value) before or instead of restoring an old image.

Restore service by redeploying the last known-good image directly through Fly, bypassing GitHub Actions (and therefore the currency guard) entirely. This targets Fly's own release history for the image, so the image choice does not depend on git state—but the `--config` argument below does, per the checkout requirement above:
