...
> **Agent:** _(writes code referencing `Sandbox.new()` — an API that doesn't exist in current SDK)_

With the skill:

> **User:** Build me a Tensorlake sandbox that runs untrusted Python and exposes port 8000.
> **Agent:** _(skill triggers → fetches `https://docs.tensorlake.ai/llms.txt` → loads `sandboxes/lifecycle.md` → writes correct code using current `tensorlake.Sandbox.create()` API)_
...
- **Skills** are better for teaching an agent _how to write code_ against an SDK — no server to run, works in any agent that supports skills, zero runtime overhead.
