...
async function getCurrentBranch(): Promise<string> {
  // Primary: works when HEAD is attached to a branch
  ...
  // Last resort: find local branches pointing at HEAD
  try {
    const refs = await sh('git', [
      'for-each-ref',
      '--points-at',
      'HEAD',
  ...
  throw new Error(
    `Could not determine current branch (detached HEAD in ${cwd}). Try: git checkout <branch>`,
  );
}
...
function formatChecksStatus(prInfo: PullRequestInfo): string {
  ...
  if (prInfo.checksStatus === 'FAILURE') {
    ...
    if (failed.length > 0) {
      ...
      result += '\nCommon commands: `bun run check`, `bun run test`, `bun run build`';
    }
...
async function main() {
  ...
  const origin = await getOriginRemoteUrl();
